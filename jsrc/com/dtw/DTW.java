...
public class DTW
{
   ...
   public static double getWarpDistBetween(TimeSeries tsI, TimeSeries tsJ)
   {
      ...
      currCol[0] = euclideanDist(tsI.getMeasurementVector(0), tsJ.getMeasurementVector(0));  // first cell
   ...
   private static TimeWarpInfo DynamicTimeWarp(TimeSeries tsI, TimeSeries tsJ)
   {
      ...
      // Calculate the values for the first column, from the bottom up.
      costMatrix[0][0] = euclideanDist(tsI.getMeasurementVector(0),
                                       tsJ.getMeasurementVector(0));
      for (int j=1; j<=maxJ; j++)
         costMatrix[0][j] = costMatrix[0][j-1] + euclideanDist(tsI.getMeasurementVector(0),
      ...
      for (int i=1; i<=maxI; i++)   // i = columns
      {
         // Calculate the value for the bottom row of the current column
         //    (i,0) = LocalCost(i,0) + GlobalCost(i-1,0)
         costMatrix[i][0] = costMatrix[i-1][0] + euclideanDist(tsI.getMeasurementVector(i),
                                                               tsJ.getMeasurementVector(0));
